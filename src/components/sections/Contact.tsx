...
import { FaGithub, FaLinkedin, FaFacebook, FaEnvelope, FaPhone, FaPaperPlane, FaCheck } from 'react-icons/fa'
...
function FloatingField({ id, label, type = 'text', value, onChange, required, placeholder, rows }: FieldProps) {
    ...
    return (
      <div className="relative">
        <textarea
          ...
          onBlur={(e: FocusEvent<HTMLTextAreaElement>) => setFocused(e.target.value.length > 0)}
  ...
  return (
    <div className="relative">
      <input
        ...
        placeholder={placeholder || label}
        onFocus={() => setFocused(true)}
        onBlur={(e: FocusEvent<HTMLInputElement>) => setFocused(e.target.value.length > 0)}
        onChange={(e) => onChange(e.target.value)}
        className={shared}
      />
      <label
  ...
  return (
    <section id="contact" className="bg-transparent py-20 md:py-28 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
        <motion.div
          animate={{ y: [0, -24, 0], opacity: [0.3, 0.5, 0.3], rotate: [0, 15, 0] }}
          transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute top-16 left-8 w-5 h-5 rounded-full bg-[#4B5694]/20"
        />
        <motion.div
          animate={{ y: [0, 18, 0], opacity: [0.2, 0.4, 0.2], rotate: [0, -20, 0] }}
        ...
        />
        <motion.div
          animate={{ y: [0, 20, 0], opacity: [0.2, 0.35, 0.2], rotate: [0, 30, 0] }}
      ...
      <div className="mx-auto max-w-6xl px-6 relative z-10">
        <SectionTitle title="Contact" subtitle="Get in touch — I'd love to hear from you" />
        ...
        <div className="grid gap-12 md:grid-cols-5">
          ...
          >
            <FloatingField id="name" label="Name" value={form.name} onChange={(v) => setForm((p) => ({ ...p, name: v }))} required placeholder="Your name" />
            <FloatingField id="email" label="Email" type="email" value={form.email} onChange={(v) => setForm((p) => ({ ...p, email: v }))} required placeholder="your.email@example.com" />
            <FloatingField id="message" label="Message" value={form.message} onChange={(v) => setForm((p) => ({ ...p, message: v }))} required placeholder="Your message..." rows={5} />
          ...
          >
            <div className="rounded-2xl bg-white/[0.06] backdrop-blur-xl border border-white/10 p-6 hover:bg-white/[0.08] transition-colors duration-300">
              ...
              <div className="space-y-4">
                ...
                >
                  <FaEnvelope className="text-[#4B5694] shrink-0 group-hover:scale-110 transition-transform duration-300" size={18} />
                ...
                {personalInfo.phone && (
                  ...
                  >
                    <FaPhone className="text-[#4B5694] shrink-0 group-hover:scale-110 transition-transform duration-300" size={18} />
